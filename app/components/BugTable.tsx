...
const groupBugs = (bugs: Bug[]) => {
  ...
  bugs.forEach((bug) => {
    const groupKey = `${bug.title} - ${bug.description} - ${
    ...
    <>
                    ...
                    {!isEditing ? (
                      <span
                        ...
                        className={`badge badge-sm badge-soft
                          ${status == 1 ? "badge-error" : "badge-success"}`}
                      >
                        {status == 1 ? (
                        ...
                        {status == 1 ? "En Attente" : "Corrigé"}
                      </span>
                    ) : (
                      <select
                        className="select select-xs select-bordered w-fit"
                        value={status}
                        onBlur={(e) => setEditingKey(null)}
                        onChange={(e) =>
      ...
      <div className="flex items-center justify-end mt-4">
        <div className="join grid grid-cols-2">
          <button onClick={goPrev} className="join-item btn btn-outline btn-sm">
            Page précédente({bugsDisplayedBefore})
          </button>
          <button onClick={goNext} className="join-item btn btn-outline btn-sm">
            Page suivante({bugsRemaining})
          </button>
        </div>
      </div>
    </>
